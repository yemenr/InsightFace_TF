...
def get_resnet(inputs, w_init=None, trainable=None, sess=None, reuse=False, keep_rate=None, weight_file=None):
    global __weights_dict
    global __is_train
    __is_train = trainable
    if (weight_file is not None) and (len(__weights_dict) == 0):
    ...
    with tf.variable_scope('', reuse=reuse):
        ...
        bn1             = tf.nn.dropout(bn1, keep_prob=keep_rate, name='E_Dropout')        
        
        pre_fc1_flatten = tf.contrib.layers.flatten(bn1)
        pre_fc1         = tf.layers.dense(pre_fc1_flatten, 512, kernel_initializer = tf.constant_initializer(__weights_dict['pre_fc1']['weights']), bias_initializer = tf.constant_initializer(__weights_dict['pre_fc1']['bias']), use_bias = True, name='pre_fc1', trainable=__is_train)
        fc1             = batch_normalization(pre_fc1, variance_epsilon=1.9999999494757503e-05, name='fc1')
        embeddings = tf.nn.l2_normalize(fc1, 1, 1e-10, name='embeddings')
    return fc1
