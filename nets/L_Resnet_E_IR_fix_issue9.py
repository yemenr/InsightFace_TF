...
def stn_process_tl(inputs, trainable):
    ...
    conv1_loc = tl.layers.Conv2d(stn_inputs, n_filter=24, filter_size=(5, 5), strides=(1, 1),
    ...
    #prelu
    prelu1_loc = tl.layers.PReluLayer(pool1_loc, name='prelu1_loc')
    
    #conv layer2: (3,3) kernels * 48, stride=1, padding=0
    conv2_loc = tl.layers.Conv2d(prelu1_loc, n_filter=48, filter_size=(3, 3), strides=(1, 1),
                                 act=None, padding='VALID', W_init=tf.contrib.layers.xavier_initializer(), 
    ...
    #prelu
    prelu2_loc = tl.layers.PReluLayer(pool2_loc, name='prelu2_loc')    
    
    #conv layer3: (3,3) kernels * 96, stride=1, padding=0
    conv3_loc = tl.layers.Conv2d(prelu2_loc, n_filter=96, filter_size=(3, 3), strides=(1, 1),
                                 act=None, padding='VALID', W_init=tf.contrib.layers.xavier_initializer(), 
    ...
    #prelu
    prelu3_loc = tl.layers.PReluLayer(pool3_loc, name='prelu3_loc')   
    ...
    fc1_loc = tl.layers.DenseLayer(pool3_loc_flat, n_units=64, W_init=tf.truncated_normal_initializer(mean=0.0,stddev=0.03), name='fc1_loc')#?,64
    #batch normal layer1                                 
    bn_fc1_loc = BatchNormLayer(fc1_loc, act=tf.identity, is_train=True, name='bn_fc1_loc', trainable=trainable)
    #prelu
    prelu_fc1_loc = tl.layers.PReluLayer(bn_fc1_loc, name='prelu_fc1_loc')   
    
    #fully connective layer: 64 output
    fc2_loc = tl.layers.DenseLayer(prelu_fc1_loc, n_units=64, W_init=tf.truncated_normal_initializer(mean=0.0,stddev=0.01), name='fc2_loc')#?,64
    #batch normal layer1                                 
    bn_fc2_loc = BatchNormLayer(fc2_loc, act=tf.identity, is_train=True, name='bn_fc2_loc', trainable=trainable)
    #prelu
    prelu_fc2_loc = tl.layers.PReluLayer(bn_fc2_loc, name='prelu_fc2_loc')
